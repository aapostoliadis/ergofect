      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {categories.map((cat, i) => (
            ...
            >
              <p className="text-[#D6FF00] bg-black inline-block px-2 py-1 font-mono text-[10px] md:text-xs font-bold uppercase mb-6">
                {cat.label}
              </p>
              <ul className="space-y-3 font-mono text-xs md:text-sm uppercase">
                {cat.tools.map((tool, j) => (
                  <li key={j} className="flex items-center gap-2 opacity-70">
                    <span className="w-1.5 h-1.5 bg-black/30 rounded-full shrink-0"></span>
                    {tool}
                  </li>
                ))}
              </ul>
